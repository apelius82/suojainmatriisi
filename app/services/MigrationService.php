...
final class MigrationService
{
    ...
    {
        $pdo->exec('CREATE TABLE IF NOT EXISTS ppe_seed_runs (id INT AUTO_INCREMENT PRIMARY KEY, filename VARCHAR(255) UNIQUE NOT NULL, executed_at DATETIME NOT NULL)');
        $done = $pdo->query('SELECT filename FROM ppe_seed_runs')->fetchAll(PDO::FETCH_COLUMN) ?: [];
        ...
        foreach ($files as $file) {
            ...
            if (trim($sql) !== '') {
                $pdo->exec($sql);
                $stmt = $pdo->prepare('INSERT INTO ppe_seed_runs (filename, executed_at) VALUES (?, NOW())');
                $stmt->execute([$filename]);
            }
        }
    }
